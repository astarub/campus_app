...
class FeedPageState extends State<FeedPage> with WidgetsBindingObserver, AutomaticKeepAliveClientMixin<FeedPage> {
  ...
  List<Widget> parsedFeedWidgets = [];
  List<Widget> searchFeedWidgets = [];

  final NewsUsecases newsUsecases = sl<NewsUsecases>();
  final CalendarUsecases calendarUsecases = sl<CalendarUsecases>();

  final FeedUtils feedUtils = sl<FeedUtils>();
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> filteredFeedItems = !explore
        ? feedUtils.filterFeedWidgets(
            filters,
            searchWord != '' ? searchFeedWidgets : parsedFeedWidgets,
          )
        : (searchWord != '' ? searchFeedWidgets : parsedFeedWidgets);
                        ...
                        child: AnimatedSwitcher(
                          ...
                          child: showSearchBar
                              ? CampusSearchBar(
                                  ...
                                  onBack: () {
                                    setState(() {
                                      searchFeedWidgets = parsedFeedWidgets;
                                      showSearchBar = false;
                                      searchWord = '';
                                    });
                                  },
  ...
  void initState() {
    ...
    final newsData = newsUsecases.getCachedFeedAndFailures();
  ...
  void onSearch(String search) {
    ...
    for (final Widget e in parsedFeedWidgets) {
    ...
    setState(() {
      searchFeedWidgets = filteredWidgets;
      searchWord = search;
    });
  }
  ...
  List<Widget> parseUpdateToWidgets() {
    ...
    return feedUtils.fromEntitiesToWidgetList(news: news, events: events);
  }
  ...
  void saveFeedExplore(int selected) {
    ...
    setState(() {
      parsedFeedWidgets = parseUpdateToWidgets();
      onSearch(searchWord);
    });
  }
